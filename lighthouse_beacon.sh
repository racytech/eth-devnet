...
SUBSCRIBE_ALL_SUBNETS=--subscribe-all-subnets
DEBUG_LEVEL=debug # possible values: info, debug, trace, warn, error, crit
...
exec $BUILD_DIR/lighthouse \
  ...
  --execution-endpoint="http://localhost:$auth_port" \
